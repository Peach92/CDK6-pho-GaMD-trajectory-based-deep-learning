...
dic=10
disx=$(echo "scale=4;$4 - $3" | bc)
disy=$(echo "scale=4;$6 - $5" | bc)
dicx=$(echo "scale=4;$disx / $dic" | bc)
dicy=$(echo "scale=4;$disy / $dic" | bc)

nlines=1500000	# number of data points used for reweighting
...
python PyReweighting-1D.py -input Phi.dat -cutoff 500 -Xdim $3 $4  -disc $dicx -Emax 50 -job amdweight_CE -weight weights.dat | tee -a reweight_variable.log
...
python PyReweighting-1D.py -input Psi.dat -cutoff 500 -Xdim $5 $6  -disc $dicy -Emax 50 -job amdweight_CE -weight weights.dat | tee -a reweight_variable.log
...
# Reweighting using cumulant expansion 
python PyReweighting-2D.py -cutoff 500 -input Phi_Psi -Xdim $3 $4 -discX $dicx -Ydim $5 $6 -discY $dicy -Emax 50 -job amdweight_CE -weight weights.dat | tee -a reweight_variable.log
